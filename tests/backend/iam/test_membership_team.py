...
class TestMembership:
    ...
    def test_update_role(self, msvc):
        ...
        updated = msvc.update_role("org-1", "u1", "admin")
        assert updated["role"] == "admin"

    def test_remove_member(self, msvc):
        msvc.add_member("org-1", "u1", "member")
        assert msvc.remove_member("org-1", "u1")
        mem = msvc.get_membership("org-1", "u1")
        assert mem["is_active"] is False

    def test_suspend_member(self, msvc):
        msvc.add_member("org-1", "u1", "member")
        assert msvc.suspend_member("org-1", "u1", "policy violation")
        mem = msvc.get_membership("org-1", "u1")
        assert mem["is_active"] is False

    def test_reactivate_member(self, msvc):
        msvc.add_member("org-1", "u1", "member")
        msvc.suspend_member("org-1", "u1", "issue")
        msvc.reactivate_member("org-1", "u1")
        assert msvc.is_member("org-1", "u1")
...
class TestTeam:
    def test_create(self, tsvc):
        team = tsvc.create("org-1", "Backend")
    ...
    def test_get_user_teams(self, tsvc):
        t1 = tsvc.create("org-1", "A")
        t2 = tsvc.create("org-1", "B")
        tsvc.add_member(t1["id"], "u1")
        tsvc.add_member(t2["id"], "u1")
        assert len(tsvc.get_user_teams("org-1", "u1")) == 2

    def test_get_child_teams(self, tsvc):
        parent = tsvc.create("org-1", "Parent")
        child = tsvc.create("org-1", "Child", parent_team_id=parent["id"])
