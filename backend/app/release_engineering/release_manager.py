"""Release Manager — plans, releases, approvals, lifecycle."""
...
class ReleaseStatus(Enum):
    PLANNED = "planned"; IN_PROGRESS = "in_progress"; APPROVED = "approved"
    DEPLOYING = "deploying"; DEPLOYED = "deployed"; ROLLED_BACK = "rolled_back"; FAILED = "failed"

@dataclass
class Release:
    id: str; org_id: str; name: str; version: str
    status: ReleaseStatus = ReleaseStatus.PLANNED; channel: str = "stable"
    description: str = ""; release_notes: str = ""; changelog: str = ""
    author_id: str = ""; reviewers: list = field(default_factory=list)
    approvers: list = field(default_factory=list); artifacts: list = field(default_factory=list)
    deployments: list = field(default_factory=list); tags: list = field(default_factory=list)
    scheduled_at: str = ""; deployed_at: str = ""; rolled_back_at: str = ""
    ...
    def from_dict(cls, data: dict) -> "Release":
        data = data.copy(); data["status"] = ReleaseStatus(data.get("status", "planned"))
        return cls(**data)

@dataclass
class ReleasePlan:
    id: str; org_id: str; name: str; description: str = ""
    releases: list = field(default_factory=list); start_date: str = ""; end_date: str = ""
    is_active: bool = True
    created_at: str = field(default_factory=lambda: datetime.now(timezone.utc).isoformat())
...
class ReleaseManager:
    def __init__(self, storage_dir: str = "release_data/releases"):
        self.storage_dir = storage_dir; self._releases: dict[str, Release] = {}
    ...
    def _load(self) -> None:
        for path, store, cls in [(self._rel_path(), self._releases, Release), (self._plan_path(), self._plans, ReleasePlan)]:
    ...
    def create_release(self, org_id: str, name: str, version: str, channel: str = "stable", author_id: str = "", description: str = "") -> Release:
        ...
        self._releases[rel.id] = rel; self._save(); return rel
    ...
    def update_status(self, rel_id: str, status: ReleaseStatus) -> Optional[Release]:
        ...
        rel.updated_at = datetime.now(timezone.utc).isoformat(); self._save(); return rel
